...
class ObjectDetection:
    ...
    def load_model(self):
        model = torch.hub.load('ultralytics/yolov5', 'custom', path=r'YOLOv5_UNO/runs/train/yolo_uno_det_color_high/weights/best.pt')
        return model
    ...
    def plot_boxes(self, results, frame):
        ...
        for i in range(n):
            ...
            if row[4] >= 0.2:
                ...
                if cardName[:4] == 'WILD':
                    possibleWildCard = cardName[5:]
                    possibleWildCard = str(possibleWildCard).replace('_', ' ')
                    cardName = possibleWildCard
                #print(cardName)
...
process_image(r'validation bilder\BLUE_s_1.jpg')
cv.waitKey(0)
