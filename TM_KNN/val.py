...
import numpy as np
import card as card
import templateMatch as TM_KNN_templateMatch
...
for file in file_dir:
    ...
    for cardImg in procImg:
        #cv.imshow('a',cardImg)
        #cv.waitKey(0)
        matchedCard = TM_KNN_templateMatch.match(cardImg)
...
np.savetxt('result/TM_KNN_Result.csv', arr_result, fmt="%s", delimiter=",")
np.savetxt('result/TM_KNN_Metrics.csv', arr_metrics, fmt="%s", delimiter=",")
